...
class Game(object):
    ...
    def Restart(self, dead=True):
        ...
        codeObj = compile("from Verbs import "+self.VerbList(), "<string>", "exec")
    ...
    def ChangeLevel(self, newLevel):
        ...
        '''
        # Reset death counter if we're changing which level we're on rather than resetting
        try:
            if self._cmdvars["level"].Name() != newLevel.Name():
                self._deathCount = 0
        except:
            pass
        # Change level in global variables
        print self.ParseText(newLevel.Text())
        newLevel.Setup(self)
